...
class ProgramFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
        foreach (self::PROGRAMS as $program ) {
            ...
            $programs ->setTitle($program[0]);
            $programs ->setSummary($program[1]);
            $programs ->setPoster($program[2]);
            $programs->setCategory($this->getReference(CategoryFixtures::CATEGORY_REFERENCE));
